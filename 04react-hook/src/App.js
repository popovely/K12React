/*
  Hook (훅)
    : 리엑트 기본 버전에서는 간단한 기능을 제작할 때만 함수를 사용했다.
      이유는 state를 변경하는 것이 클래스형 컴포넌트에서만 가능했기 떄문이다.
      하지만 16.8버전부터 Hook을 통해 함수형 컴포넌트에서도 state를 변경할 수 있게 되었다.
      React Hook은 useXXX()와 같은 이름의 함수를 사용하고,
      사용을 위해 useState를 반드시 import해야 한다.
*/
  ...
  return (
    <div className="container">
      ...
      */}
      ...
      <input type="button" value="클래스형 컴포넌트 삭제" onClick={
        function() {
          setClassShow(false);
        }
      } />
      {/* 부모에서 자식으로 데이터를 전달할 때는 props를 사용한다. */}
...
/*
  클래스형 컴포넌트
    : React.Component를 상속하여 선언한다.
      수명주기 함수중에 render()함수를 통해 렌더링하므로 필수적으로 선언해야 한다.
*/
class ClassComponent extends React.Component {
  ...
  */
  ...
  /*
    ...
    전달된 props를 통해 state를 변경하는 역할을 한다. 이 함수는 반드시 반환값이 있어야한다.
  */
  // render()가 호출되기 직전 자동호출되는 수명주기 함수
  UNSAFE_componentWillMount() {
    /*
      해당 함수는 17버전 이상에서 사용할 때는
      UNSAFE_를 붙이지 않으면 경고메세지가 뜬다.
    */
    console.error("ClassComponent => componentWillMount() 호출됨");
  }
  // render()가 호출된 직후 자동호출되는 수명주기 함수
  componentDidMount() {
    console.error("ClassComponent => componentDidMount() 호출됨");
  }
  /*
    최초 렌더링 시에는 호출되지 않고, state값이 변경에 의해 재 렌더링이 될 때 호출된다.
    해당 함수에서 true가 호출될 때만 render()가 호출된다.
  ...
  */
  ...
  /*
    클래스형 컴포넌트에서는 2개이상의 state가 필요하면 콤마(,)로 구분하여 추가하면 된다.
  */
  render() {
    ...
    return (
      <div className='container'>
        ...
        */}
        ...
        {/* 
          일반적인 함수를 사용하면 컴포넌트와 버튼을 바인딩 하기 위해 bind(this)를 사용해야 하지만,
          화살표함수(Arrow Function)를 사용하면 별도의 바인딩이 필요없다.
        */} 
      </div>
    );
  }
...
/*
  함수형 컴포넌트
    : 출력할 내용을 즉시 return하면 된다.
      함수 안에 또다른 함수를 사용할 수 없으므로 render()함수가 별도로 존재하지 않고,
      자기 자신이 render()역할을 한다.
      함수 자체가 render함수의 역할을 하므로 return만 잘해주면 된다.
*/
function FuncComponent(props) {
  ...
  */
  console.log("#Life#1", "FuncComponent ==> 함수실행");
  ...
  */
  ...
  /*
    함수형 컴포넌트에서는 state가 추가될 때마다 Hook을 통해 추가해야 한다.
  */
  // var dateState = useState((new Date()).toString());
  // var nowDate = dateState[0];   // state값
  // var setDate = dateState[1];   // state를 변경할 수 있는 함수
  // 반환값이 배열이므로 위 코드 3줄을 아래와 같이 변경할 수 있다.
  var [nowDate, setDate] = useState((new Date()).toString());

  /*
    해당 컴포넌트가 렌더링 된 후 자동으로 호출된다.
    해당 함수는 첫번째 인자로 반드시 함수가 와야한다.
    클래스형 컴포넌트는 마운팅단계와 업데이트 단계에서 사용하는 별도의 수명주기 함수가 있지만,
    함수형 컴포넌트에는 별도로 존재하지 않는다.
  */
  useEffect(function() {
    console.log("#Life#3", "FuncComponent ==> useEffect");
  });

  // 함수형 컴포넌트는 return이 될 때 렌더링 된다.
  console.log("#Life#2", "FuncComponent ==> return()실행(render와 동일)")
  return (
    <div className='container'>
      ...
      <p>Date : {nowDate}</p>
      {/*
        Hook을 통해 생성된 numberSate의 2번째 인자를 setNumber라는 함수명으로 받았으므로,
