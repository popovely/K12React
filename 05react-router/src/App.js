/*
  react-router-dom
    : 리엑트는 기본적으로 화면의 새로고침 없이 화면을 갱신한다.
      이 경우 주소가 하나로 고정되기 때문에 즐겨찾기와 같은 기능을 사용할 수 없다.
      어떤 주소로 들어왔을때 그 주소를 알아내어 그에 해당하는 컴포넌트를 렌더링하고,
      그 상태를 관리하기 위해 내부적으로 state나 props를 관리할 수 있게 해주는 도구가 react-router-dom 이다.
*/
import './App.css';
import {BrowserRouter, Route, Switch, NavLink, useParams} from "react-router-dom";
/*
  BrowserRouter
    : 리엑트 라우터 돔을 적용하고 싶은 컴포넌트의
      최상위 컴포넌트를 감싸주는 Wrapper(래퍼)로 사용한다.

  Route
    : URL에 따른 적당한 컴포넌트를 렌더링 하기위해 사용한다.
  ...
  Link
    : a태그를 통한 링크는 클릭할 때마다 화면이 새로고침되며 갱신된다.
      React는 화면의 깜빡임이 없어야 하므로 Link태그를 사용해서 링크를 눌렀을 때 reload되지 않도록 처리해준다.
*/
// 함수형 컴포넌트 정의
function Home() {
  return (
...
// Topic의 메뉴 링크를 구성하기 위한 JSON배열 생성
let contents = [
...
function Topics() {
  ...
  for (var i=0; i<contents.length; i++) {
    ...
    /*
      li태그와 같이 반복되는 엘리먼트를 사용하는 경우
      React는 중복되지 않는 key prop을 요구한다.
      그러므로 key라는 prop을 추가해야 한다.
    */
  }
  return (
    <div>
...
function Desc() {
  ...
  console.log("params", params);  // topic_cate : "qna"와 같이 출력된다.
  let topic_cate = params.topic_cate; // 파라미터 값을 얻어옴
  // 일치하는 파라미터가 없을경우 출력할 기본내용 추가
  let selected_item = {
  ...
  return (
    <BrowserRouter>
      <div className="App">
        ...
        {/* 이동하기 위한 링크 정의1 (a태그 사용) : 화면이 새로고침 된다. */}
        <ul>
          <li><a href="/">Home(a)</a></li>
          <li><a href="/Topics">Topics(a)</a></li>
          <li><a href="/Contact">Contact(a)</a></li>
        </ul>
        {/* 이동하기 위한 링크 정의2 (Link태그 사용) : 화면이 새로고침되지 않는다. */}
        <ul>
        ...
        {/*
          ...
          exact
            : <Route 컴포넌트에 삽입하면 path(경로)가 정확히 일치하는 경우에만 라우팅된다.
              만약 포함하지 않으면 Topics나 Contact를 눌렀을때도 Home이 보이는 문제가 발생한다.
        */}
        ...
        {/* 
          ...
          /가 모든 URL을 포함하는 형태이므로 아래쪽에 정의된 컴포넌트는 렌더링되지 않는다.
          즉, Home만 보이는 문제가 발생한다.
          일치하는 URL이 없는 경우에는 404메세지를 보여준다.
        */}
        <Switch>
          <Route exact path="/"><Home></Home></Route>
          <Route path="/Topics"><Topics></Topics></Route>
