...
/*
  ...
  [형식]
    import 컴포넌트명으로 사용할 이름 from '컴포넌트경로'
*/
...
// 클래스형 컴포넌트 (함수형 컴포넌트는 Hook(훅)에서 다룬다.)
class App extends Component {
  ...
  constructor(props) {
    ...
    // 해당 앱에서 사용할 데이터
    /*
      mode를 통해 첫 진입/쓰기/읽기 모드인지 판단한다.
      차후 쓰기, 삭제, 수정 등도 해당 state를 통해 판단한다.
    */
    this.state = {
  ...
  render() {
    ...
    if (this.state.mode === 'welcome') {
      ...
      // welcome일 때는 첫 진입이므로 내용이 보여야한다.
      _article = <Content title={_title} desc={_desc}></Content>;
    }
    // read(읽기)모드인 경우 contents의 첫번째 객체의 title과 desc를 보여준다.
...
//  _desc = this.state.contents[0].desc;
    ...
    _article = <Content title={_title} desc={_desc}></Content>;
      ...
      _article = <CreateForm onSubmitValue={(_title, _desc) => {
        ...
        // concat()함수는 배열을 추가하는(합치는) 기능도 가지고 있다.
        var _contents = this.state.contents.concat (
          {id:this.max_content_id, title:_title, desc :_desc}
        );
        // state값을 변경한다. 앞에서 추가한 배열로 교체되고 화면이 다시 렌더링된다.
        this.setState({
          contents : _contents,
          mode : 'read',
      ...
      /*
        기존의 내용을 읽어오기 위해 선택한 게시물의 일련번호에서 1을 차감한후
...
/* ME
      ...
      var i = 0;
      while (i < this.state.contents.length) {
        _readData = this.state.contents[i];
        if (_readData.id === this.state.selected_content_id) {
          _title = _readData.title;
          _desc = _readData.desc;
          break;
        }
        i++;
      }
...
/* T */
      ...
      // 1. 모드값을 바꾸기 위한 렌더링   / 2. 삭제처리 하기 위한 렌더링
    }
    ...
    return (
      <div className="App">
        {/* 
          ...
          해당 함수는 state중 mode를 welcome으로 변경하는 기능이 정의되었다.
        */}
        <Subject
...
//          alert("이벤트 확인용 (부모)");
        ...
        {/*
          ...
          자식은 해당 props를 호출하고, 이 때 data-id로 지정된 값을 매개변수로 전달해준다.
          이를 통해 state값을 변경한다.
        */}
...
//          alert('이벤트 확인용(Navi)');
        ...
        <Buttons onChangeMode={(btn_mode) => {
          // 여기서 삭제 처리를 하면 렌더링을 한번만해도 되므로 효율적이다.
          if(btn_mode === 'delete') {
            // 리엑트에서 confirm()을 사용할 때는 반드시 window를 붙여야한다.
            if(window.confirm('삭제할까요?')) {
              ...
              while(i < _contents.length) {
                if(_contents[i].id === this.state.selected_content_id) {
                  // scplice()를 통해 i번째 인덱스의 원소 1개를 삭제한다.
            ...
            // mode가 delete가 아닌경우 state변경후 렌더링한다.
            this.setState({
              mode : btn_mode
            });
          }
