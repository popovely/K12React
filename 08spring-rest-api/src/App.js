...
class App extends Component {
  ...
  render(){
    /*
    let content
      : mode에 따라 화면의 내용을 변경하기 위해 컴포넌트를 저장할 목적으로
        생성된 변수
        ; 컴포넌트 변수
    */
    let content;
    ...
    if(this.state.mode === 'list') {
      content = <ListContents
        /*
          ...
          이 때 부모가 내려준 props의 기능(함수)를 사용한다.
        */
      ...
      /*
        리스트에서 특정 게시물을 클릭할 때 전달된 파라미터를 통해 mode는 view로 변경하고
        일련번호는 state의 num을 변경한다.
        이 때 새롭게 렌더링 되면서 얻어온 num을 ViewContents 컴포넌트의 props로 사용한다.
      */
      content = <ViewContents num={this.state.num}
        myBoardList={(pmode)=>{
        // 리스트로 전환하기 위한 props를 정의한다.
        console.log("mode : ", pmode);
        // 파라미터로 전달받은 값을 통해 mode를 변경한 후 렌더링한다.
      ...
      content = <WriteContents
        ...
        // 폼값을 submit했을때 서버로 전송하기 위한 함수를 props로 전달
        mySubmitValue={(_id, _title, _content)=>{
          ...
          // 요청URL 선언
          let url = 'http://localhost:8081/lastjsonapi/restapi/boardWrite.do';
          /*
    ...
    return (
      <div className="container">
        ...
        <div className="row">
          {/* Left 컴포넌트 */}
          <Left></Left>
          {/* Contents 영역 (게시판 목록 or 내용보기 출력) - List, View 컴포넌트 삽입 */}
          {content}
        </div>
        <Bottom></Bottom>
      </div>
