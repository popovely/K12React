...
class ListContents extends Component {
    /*
        게시판 목록을 저장할 배열과 페이지번호 생성
        React에서는 컴포넌트의 상태를 표현하기 위해 state를 사용하고
        state값이 변경될때마다 render()함수가 호출되어 화면이 갱신된다.
    */
    state = {
        blists : [],
        pageNum : 1
    ...
    /*
    componentDidMount()
        : render()함수가 호출된 직후 호출되는 수명주기 함수.
        첫 렌더링에서는 빈화면을 먼저 출력하고,
        서버와 통신후 데이터가 있는 화면을 출력한다.
    */
    componentDidMount() {
        ...
        */
        fetch('http://localhost:8081/lastjsonapi/restapi/boardList.do?nowPage='+ pageNum) // 페이지X
        ...
        .then(function(json){
            ...
            - 위와같이 기술하면 state값을 변경할 수 있지만,
                render()함수가 호출되지 않으므로 화면이 갱신되지는 않는다.
                반드시 setState()를 통해 state값을 변경해야 render()함수가 호출되어 화면이 업데이트 된다.
            */
    ...
    render() {
        ...
        // push()를 통해 배열의 마지막부분에 데이터 삽입
        /*
        key={row.num}
            : tr태그에 key는 리액트가 중복되지 않는 key prop을 요구하므로 기술한다.
        
        data-id
            : event객체를 통해 dataset.id 속성값으로 지정된 값을 얻어올수 있다.
